...
print("Searching for mismatches (tasks.client_id != leads.client_id)...")
resp = requests.get(url + "?select=id,client_id,lead_id,leads(id,client_id)&limit=100", headers=headers)
if resp.status_code == 200:
    ...
    for row in resp.json():
        ...
        if t_cid is not None and l_cid is not None:
             if str(t_cid) != str(l_cid):
                 mismatches.append(row)
        elif t_cid is None and l_cid is not None:
            # Optionally track these too
            pass
    ...
    if mismatches:
        ...
        print("No mismatches found in the first 100 rows.")
else:
