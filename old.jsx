...
const SalesInboxPage = () => {
        ...
        const fetchInboxLeads = async () => {
            ...
            try {
                ...
                const processedLeads = (data || []).map(lead => ({
                    ...
                    // is_sender=true ÔåÆ I sent last msg | is_sender=false ÔåÆ lead sent last msg
    ...
    // 1) Prioridade Alta: confian├ºa >= 75
    // 2) Para Responder: confian├ºa < 75, lead enviou a ├║ltima msg (eu preciso responder)
    // 3) Aguardando: confian├ºa < 75, eu enviei a ├║ltima msg (aguardando lead responder)
    // 4) Stand-by: confian├ºa < 75, mais de 7 dias desde a ├║ltima mensagem
    const categorizeLeads = (leadsArray) => {
        ...
        leadsArray.forEach(lead => {
            ...
            // Rule 1: Prioridade Alta ÔÇö confian├ºa >= 75
            if (trustScore >= 75) {
                highPriority.push(lead)
                return
            }

            // Rule 4: Stand-by ÔÇö mais de 7 dias sem intera├º├úo
            ...
            if (lead._lastMsgIsSender === false) {
                // Lead enviou a ├║ltima msg ÔåÆ eu preciso responder
                toRespond.push(lead)
            } else if (lead._lastMsgIsSender === true) {
                // Eu enviei a ├║ltima msg ÔåÆ aguardando resposta do lead
                waiting.push(lead)
            } else {
                // Sem hist├│rico de intera├º├úo ÔåÆ Para Responder (default)
                toRespond.push(lead)
            }
        })
    ...
    const sendToWebhook = async (messageText) => {
        ...
        if (clientError || !client?.unipile_account_id) {
            throw new Error('N├úo foi poss├¡vel obter o account_id da Unipile para este cliente.')
        }
    ...
    const generateAISuggestion = async () => {
        ...
        try {
            ...
            if (!response.ok) throw new Error('Erro na requisi├º├úo')
            ...
            alert('ÔØî Erro ao gerar sugest├úo. Tente novamente.')
        } finally {
                ...
                <div className="flex-1 flex gap-6 overflow-hidden">
                    ...
                    <div className="w-80 flex flex-col bg-[#0d0d0d] rounded-2xl border border-white/10 overflow-hidden shrink-0">
                        <div className="p-4 border-b border-white/10 bg-[#111111] flex justify-between items-center">
                            <span className="font-semibold text-white">Inbox Priorit├írio</span>
                        ...
                        <div className="flex-1 flex flex-col bg-[#0d0d0d] rounded-2xl border border-white/10 overflow-hidden relative">
                            ...
                            {toast && (
                                <div className={`absolute bottom-20 left-1/2 -translate-x-1/2 px-4 py-2.5 rounded-xl text-sm font-medium shadow-xl backdrop-blur-md border transition-all animate-fade-in z-50 ${toast.type === 'error'
                                    ...
                                    {toast.type === 'error' ? 'ÔØî' : 'Ô£à'} {toast.message}
                                </div>
                            )}
                        </div>
                    ) : (
                    ...
                    <div className="w-80 flex flex-col gap-4 shrink-0 overflow-y-auto custom-scrollbar">
                        {activeLead ? (
                                ...
                                <div className="bg-[#0d0d0d] rounded-2xl border border-white/10 p-5">
                                    ...
                                    <div className="flex items-center justify-between gap-3">
                                        ...
                                        <div className="flex-1 text-center py-2 px-3 rounded-lg bg-blue-500/20 border border-blue-500/50 text-blue-400">
                                            <div className="text-[10px] uppercase tracking-wider opacity-70 mb-1">Intera├º├Áes</div>
                                        ...
                                        {(() => {
                                            ...
                                            return (
                                                <div className={`flex-1 text-center py-2 px-3 rounded-lg border ${style}`}>
                                                    ...
                                                    <div className="text-lg font-bold">{stage || 'ÔÇö'}</div>
                                                </div>
                                            )
                                        })()}
                                    </div>
                                ...
                                <div className="relative p-[1px] rounded-2xl bg-gradient-to-br from-primary/50 to-purple-600/50 shadow-lg shadow-primary/10">
                                    <div className="bg-[#0d0d0d] rounded-2xl p-5 h-full flex flex-col gap-4">
                                        ...
                                        {(() => {
                                            ...
                                            // Has a message (from DB or just generated) ÔåÆ show it
                                                    ...
                                                    <>
                                                        ...
                                                        {generatedReasoning && (
                                                            <div className="flex gap-2 text-gray-400 text-xs">
                                                                <span className="text-primary/60 shrink-0">­ƒÆí</span>
                                            ...
                                            // Default: Empty state ÔåÆ show generate button
                                            return (
                                                <div className="text-center py-6">
                                                    <p className="text-sm text-gray-400 mb-5">
                                                        {isIcebreaker
                                                            ? 'Nenhuma conversa iniciada com este lead.'
                                                            : 'Clique para gerar uma sugest├úo de resposta com IA.'}
                                                    </p>
                                                    <button
                                                        onClick={generateAISuggestion}
                                                        disabled={generatingReply}
                            ...
                            <div className="bg-[#0d0d0d] rounded-2xl border border-white/10 p-8 text-center text-gray-400 text-sm h-40 flex items-center justify-center">
                                Contexto do lead aparecer├í aqui.
                            </div>
                        )}
                    </div>
                </div>
